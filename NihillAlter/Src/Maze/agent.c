...
int8_t agentGetShortRoute( uint8_t gx, uint8_t gy, t_walldata *wall, uint8_t maze_scale, float *all_time, uint8_t method, uint8_t outflag, uint8_t boost )
{
  ...
  if ( counter == 5 ){
    ...
    for ( int i = 0; i < path_number; i++ ){
      if ( i == 0 && path[0] == 0 ){
        fast_path[0].distance = ADJ_FRONT_DISTANCE;
        rec_time += 0.5f;
      } else if ( path[i] > 0 ){
        if ( i == 0 ){
          fast_path[i].distance = ( ONE_BLOCK_DISTANCE * path[0] ) + ADJ_FRONT_DISTANCE;
      ...
      if ( motion_buff[i] == front ){
        if ( i == 0 ) {
          ...
          if ( fast_path[i].distance >= ONE_BLOCK_DISTANCE * 7.0f ){
            fast_path[i].speed = 700.0f + middle_boost;
            if ( fast_path[i].speed > 1000.0f ) fast_path[i].speed = 1000.0f;
          } else if ( fast_path[i].distance >= ONE_BLOCK_DISTANCE * 4.0f ){
            fast_path[i].speed = 600.0f + short_boost;
            if ( fast_path[i].speed > 900.0f ) fast_path[i].speed = 900.0f;
          } else if ( fast_path[i].distance >= ONE_BLOCK_DISTANCE * 2.0f ){
            fast_path[i].speed = 500.0f + short_boost;
            if ( fast_path[i].speed > 800.0f ) fast_path[i].speed = 800.0f;
          } else {
            fast_path[i].speed = 400.0f;
          }
          fast_path[i].end_speed = 300.0f;
        } else {
          if ( fast_path[i].distance >= ONE_BLOCK_DISTANCE * 7.0f ){
            fast_path[i].speed = 700.0f + middle_boost;
            if ( fast_path[i].speed > 1000.0f ) fast_path[i].speed = 1000.0f;
            ...
            fast_path[i].start_speed = fast_path[i-1].end_speed;
            fast_path[i].speed = 400.0f + short_boost;
            if ( fast_path[i].speed > 800.0f ) fast_path[i].speed = 800.0f;
            fast_path[i].end_speed = 300.0f;
          } else {
            fast_path[i].start_speed = fast_path[i-1].end_speed;
            fast_path[i].speed = 300.0f;
            fast_path[i].end_speed = 300.0f;
          }
        }
    ...
    fast_path[path_number].speed = 300.0f;
...
int8_t agentDijkstraRoute( int16_t gx, int16_t gy, t_walldata *wall, uint8_t maze_scale, int8_t _straight, int8_t speed_mode, int8_t out_flag )
{
  ...
  } else if ( speed_mode == PARAM_600 ){
    setFastPathParameter600( motion_buff, motion_data, &cnt_motion, out_flag );
  }
...
void setFastPathParameter600( int8_t motion_buff[256], int8_t motion_data[256], uint8_t *cnt_motion, int8_t out_flag )
{
  ...
  for( int i = 1; i < *cnt_motion; i++ ){
    if ( i == *cnt_motion -2 ){
      fast_path[i].start_speed = 600.0f;
      fast_path[i].end_speed = 0.0f;
    } else if ( i == *cnt_motion - 1 ){
      if ( motion_queue[i] == END_MOTION && out_flag == 1 ) printf( "check end motion\r\n" );
    } else {
      fast_path[i].start_speed = 600.0f;
      fast_path[i].end_speed = 600.0f;
    }

    if ( motion_buff[i] == front ){
      fast_path[i].distance = motion_data[i] * ONE_BLOCK_DISTANCE;
      // to do 距離によって変更
      if ( fast_path[i].distance > 9.0f * ONE_BLOCK_DISTANCE ) {
  ...
  if ( motion_queue[*cnt_motion-2] != SET_STRAIGHT ){
    // 終了速度を低速のまま
    fast_path[*cnt_motion-2].end_speed = 600.0f;
    // 次の動作をEND_MOTIONから直線30mm 停止へ
    motion_buff[*cnt_motion-1] = front;
    fast_path[*cnt_motion-1].distance = 10.0f;
    fast_path[*cnt_motion-1].speed = 100.0f;
    fast_path[*cnt_motion-1].start_speed = 600.0f;
    fast_path[*cnt_motion-1].end_speed = 0.0f;
    // 壁制御を有効なストレートモードを作成すること
    motion_queue[*cnt_motion-1] = SET_FRONT_PD_STRAIGHT;
    // delay
